...
export default function TransactionFilters({ filters, setFilters }: TransactionFiltersProps) {
    ...
    const [sliderValues, setSliderValues] = useState<number[]>([filters.minAmount ?? (SLIDER_MIN_VALUE + 1), filters.maxAmount ?? (SLIDER_MAX_VALUE - 1)]);
    const [selectedCategoryIds, setSelectedCategoryIds] = useState<number[]>([]);
    const [selectedTransactionGroupIds, setSelectedTransactionGroupIds] = useState<number[]>([]);
    ...
    const handleTransactionGroupSelect = (transactionGroupId: number, checked: boolean) => {
        ...
        setFilters((prev) => ({
          ...
          transactionGroupIds: checked
            ...
            : prev.transactionGroupIds?.filter((id) => id !== transactionGroupId),
        }));
    }

    const handlePaymentMethodSelect = (method: PaymentMethod, checked: boolean) => {
            ...
            <Group padding={0} className="items-center flex-wrap">
                ...
                <Group padding={0}>
                    <Stack padding={0} gap={0}>
                        <Typography className="text-xs">Amount range:</Typography>
                        <Typography className="text-xs">
                            {sliderValues[0] === SLIDER_MIN_VALUE ? "-∞" : sliderValues[0]} - {sliderValues[1] === SLIDER_MAX_VALUE ? "∞" : sliderValues[1]}
                        </Typography>
                    </Stack>
                    <Slider
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant={selectedCategoryIds.length > 0 ? "secondary" : "outline"}>
                        Filter by Category 
                        {selectedCategoryIds.length > 0 ? ': ' + categories?.filter(c => selectedCategoryIds.includes(c.id)).map(c => c.icon).join(' ') : ""}
                    </Button>
                  </DropdownMenuTrigger>
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant={selectedPaymentMethods.length > 0 ? "secondary" : "outline"}>
                        Filter by Payment Method {selectedPaymentMethods.length > 0 ? `(${selectedPaymentMethods.length})` : ""}
                    </Button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent className="w-40">
                    <DropdownMenuGroup>
                      ...
                      {Object.values(PaymentMethod)?.map((method, i) => (
                        <DropdownMenuCheckboxItem
                          ...
                          onSelect={(e) => e.preventDefault()}
                          onCheckedChange={(checked) => handlePaymentMethodSelect(method, checked)}
                        >
                            {method}
                        </DropdownMenuCheckboxItem>
                      ))}
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant={selectedTransactionGroupIds.length > 0 ? "secondary" : "outline"}>
                        Filter by Transaction Group {selectedTransactionGroupIds.length > 0 ? `(${selectedTransactionGroupIds.length})` : ""}
                    </Button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent className="w-40">
                    <DropdownMenuGroup>
                      <DropdownMenuLabel>Transaction groups</DropdownMenuLabel>
                      {transactionGroups?.map(group => (
                        <DropdownMenuCheckboxItem
                          key={group.id}
                          checked={selectedTransactionGroupIds.includes(group.id)}
                          onSelect={(e) => e.preventDefault()}
                          onCheckedChange={(checked) => handleTransactionGroupSelect(group.id, checked)}
                        >
                            {group.name}
                        </DropdownMenuCheckboxItem>
                      ))}
